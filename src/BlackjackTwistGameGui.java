...
public class BlackjackTwistGameGui {
	...
	private JTextArea textArea;// BlackjackTwistGameGui HAS-A textArea where an in game terminal will display information about the game
	...
	private boolean revealedDealerCards;// BlackjackTwistGameGui knows when to show the all the cards in the dealer's hand  
	JButton riskButton;// BlackjackTwistGameGui HAS-A button where the user can activate the risk power card
	JButton insightButton;// BlackjackTwistGameGui HAS-A button where the user can activate the insight power card
	JButton controlButton;// BlackjackTwistGameGui HAS-A button where the user can activate the control power card
	JButton cheatsButton;// BlackjackTwistGameGui HAS-A button where the user can activate cheats giving them information they
						 // shouldn't know
	...
	public BlackjackTwistGameGui() {
		...
		//Allows the user to scroll on the terminal to accommodate the potential large amounts of information  
		...
		userHealthLabel = new JLabel("Player Health: 20");
		...
		dealerHealthLabel = new JLabel("Dealer Health: 20");
		...
		// Button enabling the user to switch between the cheat terminal and the normal terminal 
	...
	/**
	 * this is responsible for adding the gui card version of a card to the user's hand 
	 * 
	...
	private void userAddCardGui(Card currCard) {
		...
		if (cardValue != 0) {
			//Adds the corresponding normal card image based on the passed card's rank suit and value
	...
	/**
	 * this is responsible aligning the power card underneath the respective power card
	 */
	...
	/**
	 * this is responsible for linking the risk,control,and insight button to a card and power button\
	 * 
	...
	private void linkPowerCardOptions(JButton powerCardButton) {
		...
		//Removes any listener's previously attached to the risk, insight, and control buttons
	...
	private void dealerCardsGui() {
		...
		//loops through the dealer's hand and add the corresponding imaged to the dealer's gui hand
		for (int i = 0; i < dealerHandSize; i++) {
			//Only the first card is revealed other wise you are just looking at the back of the card
	 ...
	 */
	private void updatePlayerCardsGui() {
	...
	private void updateGameStatus() {
		...
		if (game.isGameOver()) {
			userHealthLabel.setText("Player Health: " + game.getUser().getHealth());
			dealerHealthLabel.setText("Dealer Health: " + game.getDealer().getHealth());
			hitButton.setEnabled(false);
			standButton.setEnabled(false);
